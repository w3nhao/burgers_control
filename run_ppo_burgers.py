...
def run_ppo_burgers_closedloop():
    """
    Run PPO training using the closed-loop Burgers environment with sparse rewards
    """
    # Set up parameters
    args = Args()
    args.num_envs = 1
    ...
    args.num_minibatches = 32      # Fewer minibatches for faster training
    args.update_epochs = 10        # Update epochs
    ...
    for iteration in progress_bar:
        ...
        for step in range(args.num_steps):
            ...
            # Execute action in environment - this is a closed-loop simulation with sparse rewards
...
def run_ppo_burgers_vectorized():
    """
    Run PPO training using the vectorized Burgers environment
    """
    # Set up parameters
    args = Args()
    args.num_envs = 4  # Using 4 parallel environments
    args.total_timesteps = 200000  # Reasonable training length
    args.num_steps = 64           # Steps per rollout
    args.num_minibatches = 32      # Fewer minibatches for faster training
    args.update_epochs = 10        # Update epochs
    ...
    # Move environment to the same device if using GPU
    if device.type == "cuda":
    ...
    # Track cumulative rewards for each environment
    current_rewards = np.zeros(args.num_envs)
    current_lengths = np.zeros(args.num_envs, dtype=int)
    ...
    for iteration in progress_bar:
        ...
        for step in range(args.num_steps):
            ...
            # Execute action in all environments in parallel
            next_obs, rewards, next_done, infos = step_func(action)
            ...
            # Update cumulative rewards and lengths
            ...
            for i in range(args.num_envs):
                if next_done[i]:
                    ...
                    if 'error' in infos and i < len(infos['error']):
                        final_mse_values.append(infos['error'][i])
                    
                    # Reset cumulative stats for this environment
                    current_rewards[i] = 0
                    current_lengths[i] = 0
        ...
        # Flatten episode metrics for reporting
    ...
    # Final evaluation using a single environment for consistency
    print("\nRunning final evaluation...")
    eval_env = BurgersEnvClosedLoop()  # Use single environment for evaluation
    eval_rewards = []
    eval_mse = []
...
if __name__ == "__main__":
    # Run with the vectorized environment
    run_ppo_burgers_vectorized() 